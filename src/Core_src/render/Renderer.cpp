...
{
    if (CWin.getNativeWindow()->getInputController()->isKeyDown(window_api::LLE_KEY_R))
    { rot++; }
    if (CWin.getNativeWindow()->getInputController()->isKeyDown(window_api::LLE_KEY_T))
    { rot--; }
    ...
    projection = glm::perspective(glm::radians(45.0f),
                                  (float) CWin.getNativeWindow()->getWindowW() / CWin.getNativeWindow()->getWindowH(),
                                  0.1f, 100.0f);
